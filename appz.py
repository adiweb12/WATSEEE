...
# Directly add your Render PostgreSQL URL here for testing
# Make sure to replace 'YOUR_POSTGRESQL_RENDER_URL' with your actual connection string
...
with app.app_context():
    ...
    if not developer_user:
        new_developer_user = User(
            user_id='10000001',  # Keeping your initial developer ID
            email=developer_email,
            password="tvbroadcast67₹",
            first_name="Watcho",
            last_name="Developer",
        ...
        db.session.commit()
        print(f"Developer user '{developer_email}' ensured in DB.")
...
#--- Decorator for Developer Access ---

def developer_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        developer_email = 'watseewatcho87@gmail.com' # Define here or pass from a config
        if 'user_email' not in session or session.get('user_email') != developer_email:
            if request.path.startswith('/api/'):
                return jsonify({"success": False, "message": "Access denied: Developer privileges required."}), 403
...
def serve_static_pages(filename):
    if filename in ['search.html', 'favorites.html', 'profile.html', 'signup.html', 'index.html',
                    'continue.html','list.html', 'continue_watching.html', 'video_player.html', 'upload_video.html']:
        return render_template(filename)
    elif filename == 'developer_dashboard.html':
        developer_email = 'watseewatcho87@gmail.com'
        if 'user_email' not in session or session.get('user_email') != developer_email:
            return redirect(url_for('login_page'))
        return render_template(filename)
...
@app.route('/api/video_data/<int:video_id>') # Changed to int
def get_video_data(video_id):
    video = Video.query.get(video_id) # Query by primary key
    if video:
        video_data = {
            "id": video.id,
            "title": video.title,
            ...
            "views": f"{video.views:,} Views", # Format views back to string for display
...
def add_video():
    ...
    try:
        if views_str.endswith('M'):
            views = int(float(views_str[:-1]) * 1_000_000)
        elif views_str.endswith('K'):
            views = int(float(views_str[:-1]) * 1_000)
        else:
            views = int(views_str.split(' ')[0]) # Handle "0 Views" or just numbers
    except ValueError:
    ...
    try:
        ...
        if video_to_remove:
            ...
            def delete_uploaded_file(file_url, base_url_func, upload_folder):
                base_url = url_for(base_url_func, filename='', _external=True).rsplit('/', 1)[0] + '/'
